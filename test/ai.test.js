...
// The same JSON now carries a third field, the expansion, and parsePolishOut
// gates it through cleanExpand on the way out — so that gate and the two
...
// time this suite ran after the expansion shipped: three green-looking "falls
...
const sayDeps = (() => {
  ...
  return grab(/const AI_TELLS =[\s\S]*?;\n/) + liftFlat('findTell') +
         grab(/const EXPAND_MONEY =[\s\S]*?;\n/) + liftFlat('cleanExpand');
})();
const pctx = {};
// eslint-disable-next-line no-new-func
...
check('plain JSON comes apart',
  ...
  { text: 'Thursday works. Ill see you then.', note: 'reads a bit curt', expand: '' });
