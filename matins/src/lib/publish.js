...
import { renderIssuePage, renderArchivePage, renderTodayRedirect } from '../render/web.js';

const API = 'https://api.github.com';

async function putFile({ cfg, path, content, message, fetchImpl }) {
  ...
  else if (head.status !== 404) return { ok: false, path, error: `read ${head.status}: ${(await head.text()).slice(0, 160)}` };
...
export async function publishIssue({ issue, index, cfg, fetchImpl }) {
  ...
  return { ok: errors.length === 0, published: results.filter((r) => r.ok).map((r) => r.path), errors };
}
